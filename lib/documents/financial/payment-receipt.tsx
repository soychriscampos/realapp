...
type PaymentReceiptStudent = Pick<
  StudentDetail,
      ...
      <Page size="A4" style={financialPdfStyles.page}>
        ...
        <View style={styles.studentBlock}>
          <View style={styles.studentColumn}>
            ...
            {data.student.studentCode ? (
              <Text style={styles.muted}>
                Matrícula: {data.student.studentCode}
              </Text>
            ) : null}
          </View>
          ...
          <View style={styles.studentColumn}>
            ...
            <Text style={styles.dateValue}>
              {formatDate(data.payment.receivedAt)}
            </Text>

            <Text style={styles.folio}>
              Folio: {data.payment.id}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.amountBlock}>
          ...
          <Text style={styles.amountValue}>
            {formatCurrencyMXN(Number(data.payment.amount))}
          </Text>
        </View>
        ...
        <View style={styles.detailsTable}>
          <DetailRow
            ...
            value={data.payment.methodName}
          />

          {data.payment.bankReference ? (
            <DetailRow
              label="Referencia"
              value={data.payment.bankReference}
            />
          ) : null}

          <DetailRow
            label="Recibió"
            value={data.payment.receivedByName}
          />
        </View>
        ...
        <View style={styles.table}>
          ...
          {data.payment.allocations.map(
            (allocation, index) => (
              <View
                key={`${allocation.conceptName}-${allocation.cycleCode}-${index}`}
