...
import { chooseTutorStudent, getTutorRecentPayments, getTutorStudents, getTutorSummaries } from "@/app/tutor/data"
...
export default async function TutorPage({ searchParams }: { searchParams: Promise<{ student?: string }> }) {
  ...
  const [summaries, charges, payments] = await Promise.all([getTutorSummaries(supabase, students), Promise.all(students.map((student) => getStudentChargeBalances(supabase, student.id))), getTutorRecentPayments(supabase, students)])
