...
begin
    ...
    select e.status, gl.code, gl.is_terminal, el.code
    into v_old_status, v_grade_code, v_grade_is_terminal, v_education_level_code
    from public.enrollments e
    join public.grade_levels gl on gl.id = e.grade_level_id
    join public.education_levels el on el.id = gl.education_level_id
    ...
    if not v_grade_is_terminal or not (
        (v_education_level_code = 'PRIMARIA' and v_grade_code = '6')
        or (v_education_level_code = 'PREESCOLAR' and v_grade_code = '3')
    ) then
...
create or replace function
public.graduate_enrollment(p_enrollment_id uuid, p_effective_on date)
returns uuid
language sql
set search_path = ''
as $$
    select app_private.graduate_enrollment_internal(p_enrollment_id, p_effective_on);
$$;

revoke all on function public.graduate_enrollment(uuid, date) from public, anon;
grant execute on function public.graduate_enrollment(uuid, date) to authenticated;
