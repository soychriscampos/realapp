-- ============================================================
-- H7.3 — CONSOLIDAR PREINSCRIPCIÓN NORMAL E HISTÓRICA
-- ============================================================
...
begin
    ...
    loop
        v_guardian_id := nullif(v_contact ->> 'guardian_id', '')::uuid;
        v_full_name := nullif(btrim(v_contact ->> 'full_name'), '');
        v_phone := nullif(btrim(v_contact ->> 'phone'), '');
        v_email := nullif(btrim(v_contact ->> 'email'), '');
        v_relationship := coalesce(nullif(btrim(v_contact ->> 'relationship'), ''), 'Contacto');
    ...
    if v_has_existing_enrollment then
        ...
        into v_existing_charge_count, v_charge_id
        from public.charges c
        where c.student_id = v_student_id
          and c.cycle_id = p_target_cycle_id
          and c.financial_concept_id = v_campaign.covered_concept_id
          and c.status = 'ACTIVE';
    ...
    values (
        ...
        p_preregistered_on,
        v_charge_id,
        case when v_has_existing_enrollment then 'RESOLVED' else 'PENDING' end,
        v_actor_id,
        case when v_has_existing_enrollment then statement_timestamp() else null end,
        case when v_has_existing_enrollment then 'ENROLLED' else null end,
        nullif(btrim(p_notes), '')
    )
    returning id into v_preregistration_id;
    ...
    values (
        v_actor_id,
        case when v_has_existing_enrollment
        ...
        jsonb_build_object(
            ...
            'target_education_level_id', p_target_education_level_id,
            'target_grade_level_id', p_target_grade_level_id,
            'target_group_id', p_target_group_id,
            'charge_id', v_charge_id,
            'price', case when v_has_existing_enrollment then null else v_campaign.price end,
            'enrollment_id', case when v_has_existing_enrollment then v_enrollment.id else null end,
            'status', case when v_has_existing_enrollment then 'RESOLVED' else 'PENDING' end,
            'resolution', case when v_has_existing_enrollment then 'ENROLLED' else null end,
            'historical', v_has_existing_enrollment
        ),
        case when v_has_existing_enrollment
