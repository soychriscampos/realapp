-- Fase 1: reutilización explícita de guardians existentes en preinscripción.
...
begin
    ...
    loop
        ...
        v_relationship := nullif(btrim(v_contact ->> 'relationship'), '');
        ...
        if v_guardian_id is not null then
            ...
            select g.email into v_existing_email
            from public.guardians g
            where g.id = v_guardian_id;

            if not found then
                raise exception 'Existing guardian not found';
            end if;
            ...
            if exists (
                ...
                set
                    ...
                    priority = v_priority,
                    via_whatsapp = true,
                    via_email = coalesce((v_contact ->> 'via_email')::boolean, v_existing_email is not null),
                    is_active = true,
                    ended_at = null,
                ...
                insert into public.student_guardians (
                    student_id, guardian_id, relationship, priority,
                    via_whatsapp, via_email, is_active, started_at
                ) values (
                    v_student_id, v_guardian_id, v_relationship, v_priority,
                    true,
                    coalesce((v_contact ->> 'via_email')::boolean, v_existing_email is not null),
                    true, p_preregistered_on
                );
            end if;
