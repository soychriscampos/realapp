-- ============================================================
-- H6.1C — REACTIVACIÓN ADMINISTRATIVA + FINANCIERA
-- ============================================================

-- ------------------------------------------------------------
-- 1. Enlace canónico preinscripción -> cargo
-- ------------------------------------------------------------
...
alter table public.preregistrations
drop constraint if exists preregistrations_charge_student_fk;

alter table public.preregistrations
add constraint preregistrations_charge_student_fk
foreign key (charge_id, student_id)
references public.charges(id, student_id)
on delete restrict;

create unique index if not exists preregistrations_charge_id_uq
on public.preregistrations(charge_id)
where charge_id is not null;


-- ------------------------------------------------------------
-- 2. Helper:
--    cargo cubierto completamente por dinero/crédito no revertido
--    y sin reembolso sobre los pagos considerados.
-- ------------------------------------------------------------

create or replace function app_private.charge_is_paid_without_refund(
    p_charge_id uuid
)
returns boolean
language sql
    ...
    paid as (
        select
            ...
            coalesce((
                ...
                where capp.charge_id = p_charge_id
                  and capp.reversed_at is null
                  and cr.status = 'ACTIVE'
            ), 0) as amount_paid
    )
    select
        coalesce(paid.amount_paid >= obligation.amount_due, false)
    from obligation
    cross join paid;
$$;
...
-- ------------------------------------------------------------
-- 3. ¿La inscripción de este alumno/ciclo ya está cubierta?
--
--    Cubre si:
--    A) hay cargo ENROLLMENT_FEE del ciclo pagado sin reembolso
--    B) hay preinscripción del ciclo, ligada a cargo pagado,
--       cuya campaña cubre ENROLLMENT_FEE.
-- ------------------------------------------------------------

create or replace function app_private.enrollment_fee_is_covered(
    p_student_id uuid,
    p_cycle_id uuid
)
returns boolean
...
begin
    ...
    -- Inscripción pagada directamente.
    if exists (
        select 1
        from public.charges c
        where c.student_id = p_student_id
          and c.cycle_id = p_cycle_id
          and c.financial_concept_id = v_enrollment_fee_concept_id
    ...
    if exists (
        ...
        join public.preregistration_campaigns pc
          on pc.id = pr.campaign_id
        where pr.student_id = p_student_id
          and pr.target_cycle_id = p_cycle_id
          and pr.charge_id is not null
          and pc.covered_concept_id = v_enrollment_fee_concept_id
          and pr.status <> 'CANCELLED'
          and app_private.charge_is_paid_without_refund(pr.charge_id)
    ) then
        return true;
    end if;
...
create or replace function public.enrollment_fee_is_covered(
    p_student_id uuid,
    p_cycle_id uuid
)
returns boolean
...
grant execute
on function public.enrollment_fee_is_covered(uuid, uuid)
to authenticated;


-- ------------------------------------------------------------
-- 4. Reactivación financiera atómica
-- ------------------------------------------------------------
...
create or replace function app_private.reactivate_enrollment_financial_internal(
    ...
    -- Primer periodo de colegiatura.
    -- null cuando inicia exactamente al comienzo de un periodo.
    p_initial_tuition_amount numeric,
    p_initial_tuition_due_date date,

    -- Inscripción si no existe cobertura previa:
    -- FULL | PROPORTIONAL
...
begin
    ...
    then
        raise exception 'reactivated_on cannot precede withdrawal date';
    end if;
    ...
    -- TUITION AGREEMENT
    --
    -- Conserva el acuerdo vigente previo (precio individual,
    -- beneficio, etc.).
    -- ========================================================
    ...
    loop
        ...
        -- ----------------------------------------------------
        -- Si ya existe un cargo ACTIVO para ese mes,
        -- no generamos otro; ajustamos el del mes de reingreso
        -- si corresponde.
        -- ----------------------------------------------------
        ...
        -- ----------------------------------------------------
        -- Cargo nuevo.
        -- Puede coexistir históricamente con uno VOID anterior.
        -- ----------------------------------------------------
...
create or replace function public.reactivate_enrollment_financial(
    p_enrollment_id uuid,
    p_reactivated_on date,
    p_group_id uuid default null,
    p_economic_start_on date default null,
    p_initial_tuition_amount numeric default null,
    p_initial_tuition_due_date date default null,
...
revoke all
on function app_private.reactivate_enrollment_financial_internal(
    uuid,
    date,
    uuid,
    date,
...
grant execute
on function public.reactivate_enrollment_financial(
    uuid,
    date,
    uuid,
    date,
