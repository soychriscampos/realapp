...
begin
    ...
    select e.status, gl.is_terminal, sc.starts_on, sc.ends_on
    into v_old_status, v_grade_is_terminal, v_cycle_start, v_cycle_end
    ...
    if v_old_status not in ('ACTIVA', 'FINALIZADA') then
        raise exception 'Only ACTIVA or FINALIZADA enrollment can be graduated';
    end if;
    ...
    where id = p_enrollment_id and status in ('ACTIVA', 'FINALIZADA');
    if not found then
