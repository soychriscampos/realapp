...
begin
    ...
    for v_enrollment in
        select e.id, e.status, sc.status as cycle_status, sc.starts_on, sc.ends_on
    ...
    loop
        ...
        if v_enrollment.cycle_status <> 'ACTIVE' then
...
create or replace function
public.finalize_selected_enrollments(
    p_enrollment_ids uuid[],
    p_effective_on date
)
returns integer
...
set search_path = ''
as $$
    select app_private.finalize_selected_enrollments_internal(
        p_enrollment_ids,
        p_effective_on
    );
$$;

revoke all on function public.finalize_selected_enrollments(uuid[], date) from public, anon;
grant execute on function public.finalize_selected_enrollments(uuid[], date) to authenticated;
