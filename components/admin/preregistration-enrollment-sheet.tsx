...
export function PreregistrationEnrollmentSheet({
  ...
  const needsInitialAmount = useMemo(() => {
    if (!selected || !targetGrade || !economicStartOn) return false
    const [year, month, day] = economicStartOn.split("-").map(Number)
    if (!year || !month || !day || day === 1) return false
    const coverage = financialCoverage.find((item) => item.cycleId === selected.targetCycleId && item.educationLevelId === targetGrade.education_level_id)
    return coverage?.months.some((item) => item.year === year && item.month === month) ?? false
  }, [economicStartOn, financialCoverage, selected, targetGrade])
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await resolvePreregistrationToEnrollment({
        ...
        classesStartOn,
        economicStartOn,
        initialPeriodAmount: needsInitialAmount ? initialPeriodAmount || null : null,
        initialPeriodDueDate: needsInitialAmount ? initialPeriodDueDate || null : null,
  ...
  function formError() {
    ...
    if (needsInitialAmount && (!initialPeriodAmount.trim() || !isAmount(initialPeriodAmount))) return "Captura el importe acordado para el primer periodo."
