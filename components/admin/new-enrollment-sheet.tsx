  ...
  function stepError() {
    if (step === "student") {
      if (newStudentMode) {
        ...
        if (contacts.some((contact, index) => !contact.full_name.trim() || !contact.relationship.trim() || (contact.relationship === "Otro" && !contactOtherRelationships[index]?.trim()) || !contact.phone.trim())) {
          return "Completa nombre, parentesco y teléfono de cada contacto agregado."
        }
      } else if (!student) {
