...
export function EnrollmentActions({
  ...
  function review() {
    ...
    if (mode === "discount") {
      ...
      if (selectedDiscountCategory && !selectedDiscountVersion) {
    ...
    if (mode === "discount") {
      startTransition(async () => {
        const result = await setEnrollmentTuitionDiscount({
          enrollmentId: enrollment.id,
          studentId: enrollment.student.id,
          categoryId: selectedDiscountCategory?.id ?? null,
          effectiveOn,
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed inset-0 z-50 flex flex-col bg-white outline-none sm:left-auto sm:w-[72vw] sm:border-l sm:border-border sm:shadow-xl md:w-[68vw] lg:w-1/2">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
            ...
            {step === "review" && mode === "discount" && <DiscountReview enrollment={enrollment} category={selectedDiscountCategory} version={selectedDiscountVersion} preview={discountPreview} effectiveOn={effectiveOn} effectMode={discountEffectMode} currentPeriodAmount={discountCurrentPeriodAmount} reason={reason} />}
...
function DiscountReview({ enrollment, category, version, preview, effectiveOn, effectMode, currentPeriodAmount, reason }: {
  ...
  return <section className="space-y-5"><div><h2 className="text-lg font-semibold">Revisión del descuento</h2><p className="mt-1 text-sm text-muted-foreground">Confirma la categoría y el efecto financiero.</p></div><dl className="divide-y divide-border border-y border-border"><Summary label="Alumno" value={enrollment.student.fullName} /><Summary label="Categoría" value={category?.name ?? "Sin beneficio"} /><Summary label="Valor" value={category && version ? formatTuitionDiscountValue(version.value, category.discountType) : "Sin beneficio"} /><Summary label="Fecha efectiva" value={formatDate(effectiveOn)} /><Summary label="Efecto" value={effectLabel} />{effectMode === "PROPORTIONAL" && <Summary label="Monto final actual" value={formatCurrency(Number(currentPeriodAmount))} />}<Summary label="Colegiatura base" value={enrollment.currentTuition ? formatCurrency(enrollment.currentTuition.baseAmount) : "Sin dato"} /><Summary label="Descuento resultante" value={preview ? `-${formatCurrency(preview.discountAmount)}` : "$0.00"} /><Summary label="Monto individual resultante" value={preview ? formatCurrency(preview.agreedAmount) : enrollment.currentTuition ? formatCurrency(enrollment.currentTuition.baseAmount) : "Sin dato"} /><Summary label="Motivo" value={reason} /></dl></section>
}
