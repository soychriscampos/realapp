...
const emptyContact = (): PreregistrationIntakeContact => ({ guardianId: null, fullName: "", phone: "", email: "", relationship: "Contacto" })
...
export function PreregistrationIntakeSheet({ campaigns, cycles, levels, grades, groups, operationalCycleId, paymentContext: paymentFormContext, canReceiveForOthers }: PreregistrationIntakeSheetProps) {
  ...
  function stepError() {
    ...
    if (step === "contacts") {
      ...
      if (contacts.some((contact) => !contact.fullName.trim() || !contact.phone.trim())) return "Captura el nombre y teléfono de cada contacto."
    }
    return null
  }
...
function ContactsStep({ contacts, onUpdate, onAdd, onRemove }: { contacts: PreregistrationIntakeContact[]; onUpdate: (index: number, field: keyof PreregistrationIntakeContact, value: string) => void; onAdd: () => void; onRemove: (index: number) => void }) {
  return <section className="space-y-5"><div><h2 className="text-lg font-semibold">Contactos</h2><p className="mt-1 text-sm text-muted-foreground">Registra entre uno y dos contactos. Los datos existentes pueden corregirse.</p></div>{contacts.map((contact, index) => <div key={contact.guardianId ?? index} className="space-y-4 rounded-lg border border-border p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">Contacto {index + 1}</p>{contacts.length > 1 && <Button variant="ghost" size="sm" onClick={() => onRemove(index)}>Quitar</Button>}</div><Field label="Nombre"><Input value={contact.fullName} onChange={(event) => onUpdate(index, "fullName", event.target.value)} /></Field><Field label="Teléfono"><Input value={contact.phone} onChange={(event) => onUpdate(index, "phone", event.target.value)} inputMode="tel" /></Field><Field label="Correo"><Input value={contact.email} onChange={(event) => onUpdate(index, "email", event.target.value)} type="email" /></Field><Field label="Relación"><Input value={contact.relationship} onChange={(event) => onUpdate(index, "relationship", event.target.value)} placeholder="Ej. Madre" /></Field></div>)}{contacts.length < 2 && <Button variant="outline" onClick={onAdd}><Plus /> Agregar segundo contacto</Button>}</section>
}
