...
function SuccessActions({ paymentId, studentId, hasEmailRecipient, isSendingReceipt, onSendReceipt }: { paymentId: string | null; studentId: string; hasEmailRecipient: boolean; isSendingReceipt: boolean; onSendReceipt: () => void }) {
  ...
  return <><Button variant="outline" nativeButton={false} render={<a href={`/admin/alumnos/${studentId}/pagos/${paymentId}/recibo`} />}><Download /> Descargar</Button><Button variant="outline" disabled={!hasEmailRecipient || isSendingReceipt} onClick={onSendReceipt} title={!hasEmailRecipient ? "No hay un correo habilitado para este alumno." : undefined}>{isSendingReceipt ? <LoaderCircle className="animate-spin" /> : <Mail />}{isSendingReceipt ? "Enviando..." : "Enviar por correo"}</Button></>
}
