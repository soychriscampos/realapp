...
import { CheckCircle2, ChevronLeft, Download, LoaderCircle, Mail, Plus, Search, UserPlus, X } from "lucide-react"
...
import { sendPaymentReceiptEmail } from "@/app/admin/alumnos/[id]/payment-actions"
...
export function PreregistrationIntakeSheet({ campaigns, cycles, levels, grades, groups, operationalCycleId, paymentContext: paymentFormContext, canReceiveForOthers }: PreregistrationIntakeSheetProps) {
  ...
  function sendReceipt() {
    if (!paymentId || isSendingReceipt) return
    setIsSendingReceipt(true)
    startTransition(async () => {
      const result = await sendPaymentReceiptEmail(payment?.studentId ?? "", paymentId)
      setIsSendingReceipt(false)
      if (result.ok) {
...
function ContactsStep({ contacts, otherRelationships, onOtherChange, onUpdate, onAdd, onRemove }: { contacts: PreregistrationIntakeContact[]; otherRelationships: Record<number, string>; onOtherChange: (index: number, value: string) => void; onUpdate: (index: number, field: keyof PreregistrationIntakeContact, value: string) => void; onAdd: () => void; onRemove: (index: number) => void }) {
  ...
  const selectGuardian = (index: number, guardian: GuardianSearchResult) => {
    setSelectedGuardian(guardian)
    onUpdate(index, "guardianId", guardian.id)
    onUpdate(index, "fullName", guardian.fullName)
    onUpdate(index, "phone", guardian.phone ?? "")
    onUpdate(index, "email", guardian.email ?? "")
  }
  const clearGuardian = () => {
    if (selectedIndex === null) return
    setSelectedGuardian(null)
    onUpdate(selectedIndex, "guardianId", "")
  }
  return <GuardianContactList minimumContacts={1} contacts={contacts.map((contact) => ({ guardianId: contact.guardianId, fullName: contact.fullName, phone: contact.phone, email: contact.email, relationship: contact.relationship }))} onChange={(nextContacts) => onUpdateContacts(nextContacts)} />
  function onUpdateContacts(nextContacts: Array<{ guardianId: string | null; fullName: string; phone: string; email: string; relationship: string }>) {
    const next = nextContacts.map((contact) => ({ guardianId: contact.guardianId, fullName: contact.fullName, phone: contact.phone, email: contact.email, relationship: contact.relationship }))
    while (next.length > contacts.length) onAdd()
    next.forEach((contact, index) => {
      onUpdate(index, "guardianId", contact.guardianId ?? "")
      onUpdate(index, "fullName", contact.fullName)
      onUpdate(index, "phone", contact.phone)
      onUpdate(index, "email", contact.email)
      onUpdate(index, "relationship", contact.relationship)
    })
    for (let index = contacts.length - 1; index >= next.length; index -= 1) onRemove(index)
  }
  return <section className="space-y-5"><div><h2 className="text-lg font-semibold">Contactos</h2><p className="mt-1 text-sm text-muted-foreground">Crea un contacto nuevo o vincula un tutor existente.</p></div>{contacts.map((contact, index) => { const relationshipValue = isGuardianRelationship(contact.relationship) ? contact.relationship : contact.relationship ? "Otro" : ""; const selected = selectedIndex === index ? selectedGuardian : null; return <div key={contact.guardianId ?? index} className="space-y-4 rounded-lg border border-border p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">Contacto {index + 1}</p>{contacts.length > 1 && <Button variant="ghost" size="sm" onClick={() => onRemove(index)}>Quitar</Button>}</div><GuardianPicker selected={selected} onSelect={(guardian) => { setSelectedIndex(index); selectGuardian(index, guardian) }} onClear={clearGuardian} /><Field label="Nombre"><Input value={contact.fullName} disabled={Boolean(selected)} onChange={(event) => onUpdate(index, "fullName", event.target.value)} /></Field><Field label="Teléfono"><Input value={contact.phone} disabled={Boolean(selected)} onChange={(event) => onUpdate(index, "phone", event.target.value)} inputMode="tel" /></Field><Field label="Correo"><Input value={contact.email} disabled={Boolean(selected)} onChange={(event) => onUpdate(index, "email", event.target.value)} type="email" /></Field><Field label="Parentesco"><select value={relationshipValue} onChange={(event) => onUpdate(index, "relationship", event.target.value === "Otro" ? "Otro" : event.target.value)} className={selectClass}><option value="">Selecciona un parentesco</option>{GUARDIAN_RELATIONSHIPS.map((relationship) => <option key={relationship} value={relationship}>{relationship}</option>)}</select></Field>{relationshipValue === "Otro" && <Field label="Especificar parentesco"><Input value={otherRelationships[index] ?? ""} onChange={(event) => { onOtherChange(index, event.target.value); onUpdate(index, "relationship", "Otro") }} /></Field>}</div>})}{contacts.length < 2 && <Button variant="outline" onClick={onAdd}><Plus /> Agregar segundo contacto</Button>}</section>
}
...
function SuccessStep({ paid, historical }: { paid: boolean; historical: false | "PENDING" | "COVERED" | "UNLINKED" | "REVIEW"; paymentId?: string | null; studentId?: string; hasEmailRecipient?: boolean; isSendingReceipt?: boolean; onSendReceipt?: () => void }) {
  const historicalStatus = historical || null
  const message = paid ? "El pago se registró y aplicó al cargo de preinscripción." : historicalStatus === "COVERED" ? "La inscripción ya está cubierta. No se ofreció otro cobro." : historicalStatus === "UNLINKED" || historicalStatus === "REVIEW" ? "La preinscripción fue registrada, pero no se encontró una obligación financiera inequívoca para vincular. Requiere revisión financiera." : historicalStatus ? "El antecedente histórico quedó registrado sobre la matrícula existente. No se generó un cargo nuevo." : "La preinscripción y el cargo de campaña quedaron registrados."
  return <section className="py-8 text-center"><CheckCircle2 className="mx-auto size-10 text-emerald-600" /><h2 className="mt-4 text-lg font-semibold">Alumno preinscrito correctamente</h2><p className="mt-2 text-sm text-muted-foreground">{message}</p></section>
}

function SuccessActions({ paymentId, studentId, hasEmailRecipient, isSendingReceipt, onSendReceipt }: { paymentId: string | null; studentId: string; hasEmailRecipient: boolean; isSendingReceipt: boolean; onSendReceipt: () => void }) {
  if (!paymentId) return null
  return <><PaymentReceiptDownloadButton href={`/admin/alumnos/${studentId}/pagos/${paymentId}/recibo`}><Download /> Descargar</PaymentReceiptDownloadButton><Button variant="outline" disabled={!hasEmailRecipient || isSendingReceipt} onClick={onSendReceipt} title={!hasEmailRecipient ? "No hay un correo habilitado para este alumno." : undefined}>{isSendingReceipt ? <LoaderCircle className="animate-spin" /> : <Mail />}{isSendingReceipt ? "Enviando..." : "Enviar por correo"}</Button></>
}
